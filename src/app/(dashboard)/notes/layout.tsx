...
export default function NotesLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      ...
      <div className="glass-panel flex h-[calc(100vh-130px)] -mx-4 sm:-mx-6 lg:-mx-8 -my-8 overflow-hidden">
          ...
          <div className="p-4 border-b border-white/[0.04]">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="relative group">
                ...
                <div className="glass-panel absolute right-0 top-full mt-1 w-40 overflow-hidden rounded-xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                  {(['last-edited', 'created', 'alphabetical'] as SortOption[]).map(opt => (
                    <button
                      key={opt}
                      onClick={() => setSortOption(opt)}
                      className={`w-full text-left px-3 py-2 text-xs transition-colors ${sortOption === opt ? 'bg-white/[0.08] text-white' : 'text-white/60 hover:bg-white/[0.04] hover:text-white'}`}
                    >
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center h-40 text-center px-4">
                ...
                {!searchQuery && !showArchived && (
                  <button onClick={handleCreateNote} className="mt-3 text-xs text-white/70 hover:text-white/50 font-medium">
              ...
              filteredAndSortedNotes.map(note => (
                ...
                >
                  <h3 className={`text-sm font-medium truncate mb-1 ${activeNoteId === note.id ? 'text-white' : 'text-white/70'}`}>

...
export default function CalendarPage() {
  ...
  const getPriorityColor = (priority: string) => {
    switch(priority) {
      case 'high': return 'bg-white/[0.08] text-white/70 border-white/10';
      case 'medium': return 'bg-white/[0.08] text-white/70 border-white/10';
      default: return 'bg-white/[0.08] text-white/50 border-white/20/30';
    }
  };
  ...
  return (
    <div className="flex h-full w-full">
      ...
      <div className="glass-panel w-64 border-t-0 border-b-0 border-l-0 border-r flex flex-col p-4 rounded-none">
        ...
        <button
          ...
          className="w-full mb-6 bg-white text-black hover:bg-white/90 rounded-xl py-2.5 px-4 text-sm font-medium flex items-center justify-center gap-2 transition-colors shadow-lg shadow-white/10"
        >
          <PlusIcon size={16} />
          New Event
        </button>
          ...
          <div>
            <div className="relative">
              ...
              <input 
                ...
                className="w-full bg-white/[0.04] border border-white/5 rounded-lg pl-9 pr-3 py-2 text-sm text-white placeholder-white/30 focus:outline-none focus:border-white/20 focus:bg-white/[0.06] transition-all"
              />
            </div>
          </div>
            ...
            <div className="space-y-3">
              <label className="block">
                ...
                <select 
                  ...
                  className="w-full bg-white/[0.04] border border-white/5 rounded-lg px-3 py-1.5 text-sm text-white/80 focus:outline-none focus:border-white/20 [&>option]:bg-[#1a1a24] [&>option]:text-white"
                >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Toolbar */}
        <div className="glass-panel h-16 border-l-0 border-r-0 border-t-0 border-b flex items-center justify-between px-6 z-10 relative rounded-none">
                    ...
                    >
                      <div className="flex justify-between items-start mb-4">
                        <span className={`text-sm font-medium w-8 h-8 flex items-center justify-center rounded-full ${
                          isToday ? 'bg-white text-black shadow-lg shadow-white/10' : 'text-white/70'
                        }`}>
                          {dayObj.date.getDate()}
                        </span>
                      </div>
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className={`w-1 h-12 rounded-full ${
                          event.priority === 'high' ? 'bg-white/10' :
                          event.priority === 'medium' ? 'bg-white text-black' : 'bg-white text-black'
                        }`} />
                        <div>
                      ...
                      <div className="flex items-center gap-3">
                        <span className={`px-2.5 py-1 rounded-md text-[10px] font-medium uppercase tracking-wider border ${
                          event.status === 'completed' ? 'bg-white/[0.08] text-white/70 border-white/20/30' :
                          event.status === 'in-progress' ? 'bg-white/[0.08] text-white/70 border-white/20/30' :
                        ...
                        <div className="opacity-0 group-hover:opacity-100 flex items-center transition-opacity bg-black/40 rounded-lg p-1">
                          ...
                          <button 
                            ...
                            className="p-1.5 text-white/40 hover:text-white/70 hover:bg-white/[0.08] rounded transition-colors ml-1"
                            title="Delete"
                          >
                            <TrashIcon size={14} />
                          </button>

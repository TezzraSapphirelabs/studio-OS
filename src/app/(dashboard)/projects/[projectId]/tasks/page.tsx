...
const PRIORITY_CONFIG = {
  high: { label: 'High', color: 'text-white/70', bg: 'bg-white/[0.04]', icon: ArrowUpIcon },
  medium: { label: 'Medium', color: 'text-white/70', bg: 'bg-white/[0.04]', icon: ArrowRightIcon },
  low: { label: 'Low', color: 'text-white/70', bg: 'bg-white/[0.04]', icon: ArrowDownIcon },
};
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <input
              ...
              className="h-11 w-full rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 text-sm text-white outline-none transition-all focus:border-white/20 focus:bg-white/[0.06] focus:ring-1 focus:ring-white/20/25"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full resize-none rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 py-3 text-sm text-white outline-none transition-all focus:border-white/20 focus:bg-white/[0.06] focus:ring-1 focus:ring-white/20/25"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                className="h-11 w-full rounded-xl border border-white/[0.08] bg-[#12121a] px-4 text-sm text-white outline-none transition-all focus:border-white/20 focus:ring-1 focus:ring-white/20/25"
              >
          ...
          <div>
            ...
            <input
              ...
              className="h-11 w-full rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 text-sm text-white outline-none transition-all focus:border-white/20 focus:bg-white/[0.06] focus:ring-1 focus:ring-white/20/25 [color-scheme:dark]"
            />
          </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={saving || !title.trim()}
              className="flex-1 h-11 rounded-xl bg-white  text-sm font-semibold  text-black shadow-lg shadow-white/10 transition-all hover:shadow-white/20 hover:brightness-110 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed">
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
        ...
        <div className="flex flex-1 items-center gap-3">
          <div className="relative max-w-sm flex-1">
            ...
            <input
              ...
              className="h-10 w-full rounded-xl border border-white/[0.08] bg-white/[0.04] pl-10 pr-4 text-sm text-white placeholder:text-white/25 outline-none transition-all focus:border-white/20 focus:bg-white/[0.06] focus:ring-1 focus:ring-white/20/25"
            />
          </div>
            ...
            {(['all', 'todo', 'in-progress', 'done'] as const).map((s) => (
              <button
                ...
                className={`rounded-lg px-3 py-1.5 text-xs font-medium capitalize transition-all ${
                  statusFilter === s ? 'bg-white text-black/20 text-white/50 shadow-sm' : 'text-white/40 hover:bg-white/[0.04] hover:text-white/60'
                }`}
              >
                {s.replace('-', ' ')}
              </button>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-6 text-center text-white/70">
          {error}
        </div>
      ) : tasks.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <div className="mb-5 flex h-16 w-16 items-center justify-center rounded-2xl bg-white/[0.04] text-white/70">
          ...
          {canCreateTask && (
            <button onClick={() => setShowModal(true)} className="inline-flex h-10 items-center gap-2 rounded-xl bg-white  px-5 text-sm font-semibold  text-black shadow-lg transition-all hover:brightness-110 active:scale-[0.98]">
              ...
              <GlassCard key={task.id} hover padding="sm" className="group flex flex-col gap-4 sm:flex-row sm:items-center">
                ...
                <div className="flex flex-1 items-start gap-4">
                  <button
                    ...
                    className={`mt-1 shrink-0 rounded-full transition-all hover:scale-110 disabled:opacity-50 disabled:hover:scale-100 ${task.status === 'done' ? 'text-white/70' : 'text-white/20 hover:text-white/40'}`}
                  >
                ...
                <div className="flex items-center gap-4 sm:ml-auto">
                  ...
                  <div className="flex items-center gap-2 border-l border-white/[0.08] pl-4 opacity-0 transition-opacity group-hover:opacity-100 focus-within:opacity-100 sm:pl-4">
                    ...
                    {canDeleteTask && (
                      <button onClick={() => handleDelete(task)} className="p-1.5 text-white/70/50 hover:text-white/70 transition-colors">
                        <TrashIcon size={14} />
                      </button>
                    )}
                  </div>
